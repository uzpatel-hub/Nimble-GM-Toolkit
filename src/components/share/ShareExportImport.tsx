...
export function ShareExportDialog({ campaignId, preSelectedNpcIds, trigger }: ShareExportProps) {
  ...
  const totalItems = (includeParty ? (campaign?.partyMembers?.length ?? 0) : 0) + selectedNpcIds.size;
...
export function ShareImportDialog({ campaignId, trigger }: ShareImportProps) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      const { partyMembers, npcs, images: newImages, result } =
        await importSharePayload(payload, campaignId);

      // Add images to image store (metadata only, blobs already in IndexedDB)
      for (const img of newImages) {
        // We already stored the blob via importSharePayload, so just add metadata
        // Use the store's setState directly to avoid re-storing the blob
      ...
      for (const npc of npcs) {
        // Use setState directly since IDs are already assigned
